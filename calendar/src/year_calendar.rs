...
impl YearCalendar {
    ...
    pub fn from_calendar(
        ...
        year: i32,
        verbose: bool,
    ) -> Result<Self, crate::calendar::CalendarError> {
        let y = Year::new(year);
        let mut ycal = Self {
            year: y.clone(),
            province: calendar.province,
        ...
        for d in dates {
            ...
            for ye in yee {
                ...
                let rhd = ReportHolyday {
                    ...
                    death: ye.holyday.death().to_string(),
                    attrs: ye
                        .holyday
                        .main()
                        .iter()
    ...
    fn add(
        ...
        if let Some(day_holydays) = self.holydays_by_date.get_mut(&ye.date) {
            ...
            self.holydays_by_date.insert(ye.date, de); // may insert empty list,
                                                       // is ok
        };
        Ok(())
    }
    ...
    pub fn fix_holyday_date_is_ok(
        ...
        //     _ => true,
        // };
        let is_in_advent = ye.date >= year.advent_next
            && ye.date < NaiveDate::from_ymd_opt(year.ad, 12, 25).unwrap();
        if is_in_advent {
        ...
        //        let is_in_holy_week = ye.date >= year.palm_sunday && ye.date <
        // year.easter;
        ...
        match t {
            ...
            calendar::TransferType::Normal => match c {
                ...
                calendar::HolydayClass::Principal => {
                    ...
                    // holydays.",     //     ye.holyday.borrow().title
                    //     // )))
                    //     DropStatus::Drop(DropReason::Clash) // ??
                    // } else {
                    DropStatus::Keep
            ...
            calendar::TransferType::Joseph => {
                if is_in_easter {
                    let abvm = NaiveDate::from_ymd_opt(year.ad, 3, 25).unwrap();
...
struct ReportHolyday {
    title: String,
    description: String, // is constructed
    class_format: String,
    other: Vec<String>,
...
impl Year {
    ...
    pub fn new(year: i32) -> Self {
        let easter = NaiveDate::from_ymd_opt(year, 3, 1).unwrap()
    ...
    fn date_cal_to_date(&self, date_cal: &calendar::DateCal) -> NaiveDate {
        match date_cal {
            ...
            calendar::DateCal::Fixed { month, day } => {
                NaiveDate::from_ymd_opt(self.ad, u32::from(*month), u32::from(*day)).unwrap()
    ...
    pub fn season_colour(&self, date: NaiveDate) -> calendar::SeasonColour {
        if date >= NaiveDate::from_ymd_opt(self.ad, 12, 25).unwrap()
            || date <= NaiveDate::from_ymd_opt(self.ad, 2, 2).unwrap()
            || (date >= self.easter && date < self.pentecost)
            || date == NaiveDate::from_ymd_opt(self.ad, 11, 1).unwrap()
        ...
        else if (date >= self.advent_next
            && date < NaiveDate::from_ymd_opt(self.ad, 12, 25).unwrap())
...
mod tests {
    use super::*;
    use crate::{
        calendar::{DateCal, Holyday, TransferType},
        // year_calendar::{Year, YearCalendar, YearHolyday},
    };
    ...
    fn test_easter() {
        ...
        let er = YearCalendar::fix_holyday_date_is_ok(&day_holydays, &mut ye, &year);
        assert_eq!(DropStatus::Keep, er);
        assert_eq!(NaiveDate::from_ymd_opt(2019, 4, 21), Some(ye.date));
    }
